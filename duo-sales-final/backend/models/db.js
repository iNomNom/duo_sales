...
db.exec(`
  CREATE TABLE IF NOT EXISTS users (
    ...
    sales_cycle_start INTEGER DEFAULT 7,       -- Day of month when sales cycle starts (1-28)
...
if (!hasSalesCycle && !hasOldBillingCycle) {
  db.exec('ALTER TABLE users ADD COLUMN sales_cycle_start INTEGER DEFAULT 7');
...
// Set all other agents to 7 (default) if not set
db.prepare("UPDATE users SET sales_cycle_start = 7 WHERE role = 'agent' AND sales_cycle_start IS NULL").run();
...
if (!existing) {
  ...
  db.prepare(`
    ...
    VALUES ('Admin', 'admin@duoenterprizes.com', ?, 'admin', 7)
  `).run(hash);
...
module.exports = { db, getSalesPeriod, getAgentSalesCycles };
