...
function AgentDetailModal({ agent, onClose }) {
  ...
  useEffect(() => { loadAgentData('monthly'); }, [agent.name]);

  const applyFilter = (filter) => { setActiveFilter(filter); if (filter !== 'custom') loadAgentData(filter); };
  const applyCustomDate = () => { if (customFrom && customTo) loadAgentData('custom', customFrom, customTo); };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ position: 'relative', width: '90%', maxWidth: 1000, maxHeight: '85vh', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 25px 50px rgba(0,0,0,0.5)' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--accent2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 600, color: '#fff' }}>{agent.name?.[0]?.toUpperCase()}</div>
              <div>
                <h2 style={{ fontSize: 18, fontWeight: 600, color: 'var(--text)', margin: 0 }}>{agent.name}</h2>
                <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>{agent.email} · {periodLabel}</div>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
            {[
              ...
              <button key={f.value} onClick={() => applyFilter(f.value)} style={{
                padding: '5px 10px', borderRadius: 6, fontSize: 11, fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s',
            ...
            {activeFilter === 'custom' && (
              <>
                <input type="date" value={customFrom} onChange={e => setCustomFrom(e.target.value)} style={{ background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 6, padding: '5px 8px', color: 'var(--text)', fontSize: 11 }} />
                <span style={{ color: 'var(--muted)', fontSize: 11 }}>to</span>
                <input type="date" value={customTo} onChange={e => setCustomTo(e.target.value)} style={{ background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 6, padding: '5px 8px', color: 'var(--text)', fontSize: 11 }} />
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '20px 24px' }}>
          {loading ? (
            ...
            <>
              ...
              {/* Sales table — grouped by cycle month */}
              ...
              {data.sales.length === 0 ? (
                ...
                // Group sales by cycle month
                const grouped = {};
                data.sales.forEach(s => {
                  const cycleMonth = s.cycle_period_start ? s.cycle_period_start.substring(0, 7) : 'unknown';
                  if (!grouped[cycleMonth]) {
                    grouped[cycleMonth] = { monthKey: cycleMonth, cycleFormat: s.cycle_format || '', sales: [] };
                  }
                  grouped[cycleMonth].sales.push(s);
                });
                const sortedGroups = Object.values(grouped).sort((a, b) => (b.monthKey || '').localeCompare(a.monthKey || ''));

                const cycleFormat = data.salesCycle?.cycle_format || '?';

                return sortedGroups.map(group => (
                  <div key={group.monthKey} style={{ marginBottom: 16 }}>
                    <div style={{
                      ...
                      <span style={{ fontSize: 10, color: 'var(--muted)' }}>
                        {agent.name} ({cycleFormat}) · {group.sales.length} sales · Net: ${group.sales.reduce((sum, s) => {
...
function PasswordChangeModal({ user, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    } catch (err) { setMsg(err.response?.data?.error || 'Error updating password'); }
  };

  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)' }} />
      <div style={{ position: 'relative', width: '90%', maxWidth: 400, background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14, padding: 24, boxShadow: '0 25px 50px rgba(0,0,0,0.5)' }}>
...
export default function Agents() {
  ...
  const load = async () => {
    try {
      ...
      if (activeFilter === 'custom' && customFrom && customTo) { params.from = customFrom; params.to = customTo; }
  ...
  const applyCustomDate = () => { if (customFrom && customTo) load(); };
  ...
  const deleteUser = async (id) => { if (!window.confirm('Delete this user?')) return; await axios.delete(`/api/auth/users/${id}`); load(); };
    ...
    <div style={{ padding: 28 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ color: 'var(--muted)', fontSize: 13, marginTop: 4 }}>Showing: {filterLabel} · Click agent card to view details</p>
        </div>
        <button onClick={() => setShowAdd(!showAdd)} style={{ padding: '8px 16px', background: 'linear-gradient(135deg,#4f8ef7,#6c63ff)', border: 'none', borderRadius: 8, color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>+ Add User</button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        {activeFilter === 'custom' && (
          <>
            <input type="date" value={customFrom} onChange={e => setCustomFrom(e.target.value)} style={{ background: 'var(--bg2)', border: '1px solid var(--border2)', borderRadius: 8, padding: '6px 10px', color: 'var(--text)', fontSize: 12 }} />
            <span style={{ color: 'var(--muted)', fontSize: 12 }}>to</span>
            <input type="date" value={customTo} onChange={e => setCustomTo(e.target.value)} style={{ background: 'var(--bg2)', border: '1px solid var(--border2)', borderRadius: 8, padding: '6px 10px', color: 'var(--text)', fontSize: 12 }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14, marginBottom: 24 }}>
        {agents.map(a => (
          <div key={a.id || a.name} onClick={() => setSelectedAgent(a)} style={{
            background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: 20, cursor: 'pointer', transition: 'all 0.15s',
          }}
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--accent2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 600, color: '#fff', flexShrink: 0 }}>{a.name?.[0]?.toUpperCase()}</div>
            ...
            {(a.cancelled_amount > 0 || a.chargeback_amount > 0) && (
              <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
                {a.cancelled_amount > 0 && <span style={{ fontSize: 10, color: '#f87171', background: 'rgba(248,113,113,0.1)', padding: '2px 6px', borderRadius: 4 }}>Cancelled: ${Number(a.cancelled_amount || 0).toLocaleString()}</span>}
                {a.chargeback_amount > 0 && <span style={{ fontSize: 10, color: '#a78bfa', background: 'rgba(167,139,250,0.1)', padding: '2px 6px', borderRadius: 4 }}>Chargeback: ${Number(a.chargeback_amount || 0).toLocaleString()}</span>}
              </div>
            )}
      ...
      {/* Password Change Modal */}
      {passwordChangeUser && <PasswordChangeModal user={passwordChangeUser} onClose={() => setPasswordChangeUser(null)} />}
    </div>
  );
}
