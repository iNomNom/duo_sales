...
export default function Settings() {
  ...
  const changePassword = async (e) => {
    ...
    try {
      ...
      setPwMsg(err.response?.data?.error || 'Error updating password');
    }
  };
...
const downloadBackup = async () => {
  ...
  return (
    <div style={{ padding: 28, maxWidth: 700 }}>
      ...
      {/* Change password - Available to ALL users (including agents) */}
      <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 24, marginBottom: 16 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)', marginBottom: 16 }}>Change Password</div>
        {pwMsg && <div style={{ background: pwMsg.includes('updated') || pwMsg.includes('successfully') ? 'rgba(52,211,153,0.1)' : 'rgba(248,113,113,0.1)', border: `1px solid ${pwMsg.includes('updated') || pwMsg.includes('successfully') ? 'rgba(52,211,153,0.3)' : 'rgba(248,113,113,0.3)'}`, borderRadius: 8, padding: '10px 14px', color: pwMsg.includes('updated') || pwMsg.includes('successfully') ? 'var(--green)' : 'var(--red)', fontSize: 13, marginBottom: 16 }}>{pwMsg}</div>}
        <form onSubmit={changePassword}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 16 }}>
            ...
            <div><label style={LABEL}>New Password</label><input type="password" value={pwForm.newPassword} onChange={e => setPwForm(f => ({ ...f, newPassword: e.target.value }))} required minLength={6} style={INPUT} /></div>
            <div><label style={LABEL}>Confirm New</label><input type="password" value={pwForm.confirm} onChange={e => setPwForm(f => ({ ...f, confirm: e.target.value }))} required minLength={6} style={INPUT} /></div>
          </div>
