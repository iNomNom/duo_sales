...
function EditModal({ sale, onSave, onCancel }) {
  ...
  const fields = [
    ...
    { section: 'Notes', items: [{ key: 'notes', label: 'Notes', type: 'textarea' }] },
  ];
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ position: 'relative', width: '90%', maxWidth: 800, maxHeight: '85vh', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 25px 50px rgba(0,0,0,0.5)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                {section.items.map(item => (
                  <div key={item.key} style={item.type === 'textarea' ? { gridColumn: '1 / -1' } : {}}>
                    <label style={{ display: 'block', fontSize: 10, color: 'var(--muted)', fontWeight: 500, marginBottom: 4, textTransform: 'uppercase' }}>{item.label}</label>
                    {item.type === 'select' ? (
                      <select value={form[item.key] || ''} onChange={e => set(item.key, e.target.value)} style={INPUT}>{item.options.map(o => <option key={o}>{o}</option>)}</select>
                    ) : item.type === 'textarea' ? (
...
function ColumnPicker({ columns, visibleColumns, onToggle }) {
  ...
  return (
    <div style={{ position: 'relative' }} ref={ref}>
      ...
      {open && (
        <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 4, zIndex: 100, background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: 12, minWidth: 200, maxHeight: 400, overflow: 'auto', boxShadow: '0 10px 30px rgba(0,0,0,0.4)' }}>
          ...
          {columns.map(col => (
            <label key={col.key} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 4px', cursor: 'pointer', borderRadius: 4 }}
              onMouseEnter={e => e.currentTarget.style.background = 'var(--bg3)'}
              onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
              <input type="checkbox" checked={visibleColumns.includes(col.key)} onChange={() => onToggle(col.key)} style={{ accentColor: 'var(--accent)', cursor: 'pointer' }} />
...
export default function Sales() {
  ...
  const [visibleColumns, setVisibleColumns] = useState(ALL_COLUMNS.filter(c => c.visible).map(c => c.key));
  ...
  const deleteSale = async (id) => { if (!window.confirm('Delete this sale?')) return; await axios.delete(`/api/sales/${id}`); load(); };
  const saveEdit = async (form) => { await axios.put(`/api/sales/${editSale.id}`, form); setEditSale(null); load(); };
  const changeStatus = async (id, status) => { if (!window.confirm(`Change status to ${status}?`)) return; try { await axios.patch(`/api/sales/${id}/status`, { status }); load(); } catch {} };
  ...
  const toggleColumn = (key) => {
    if (key === 'all') setVisibleColumns(ALL_COLUMNS.map(c => c.key));
    else if (key === 'defaults') setVisibleColumns(ALL_COLUMNS.filter(c => c.visible).map(c => c.key));
    else setVisibleColumns(prev => prev.includes(key) ? prev.filter(k => k !== key) : [...prev, key]);
  };
  ...
  const renderCell = (s, col) => {
    ...
    if (col.key === 'status') return <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 20, fontWeight: 500, background: (STATUS_COLORS[val] || '#888') + '22', color: STATUS_COLORS[val] || 'var(--muted)' }}>{val}</span>;
  ...
  // Group sales by cycle month (YYYY-MM from cycle_period_start)
  const groupedSales = {};
  sales.forEach(s => {
  ...
  // Build sub-info showing each cycle format with agent names and date ranges
    ...
    <div style={{ padding: 28 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button onClick={() => setGroupByCycle(!groupByCycle)}
            style={{ padding: '7px 14px', background: groupByCycle ? 'var(--accent)' : 'transparent', border: groupByCycle ? '1px solid var(--accent)' : '1px solid var(--border2)', borderRadius: 8, color: groupByCycle ? '#fff' : 'var(--muted)', fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select value={filters.status} onChange={e => setF('status', e.target.value)} style={INPUT_S}>
          <option value="">All Statuses</option><option>Active</option><option>Pending</option><option>Cancelled</option><option>Chargeback</option>
        </select>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  {gi === 0 && (
                    <thead><tr style={{ borderBottom: '1px solid var(--border)' }}>
                      {visibleCols.map(col => (<th key={col.key} style={{ textAlign: 'left', padding: '10px 12px', fontSize: 11, fontWeight: 500, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.3px', whiteSpace: 'nowrap', minWidth: col.width }}>{col.label}</th>))}
                  ...
                  <tbody>
                    {group.sales.map(s => (
                      <tr key={s.id} style={{ borderBottom: '1px solid var(--border)', transition: 'background 0.1s' }}
                        ...
                        onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                        {visibleCols.map(col => (<td key={col.key} style={{ padding: '10px 12px', color: col.key === 'date' ? 'var(--muted)' : 'var(--text)', fontSize: col.key === 'date' ? 12 : undefined }}>{renderCell(s, col)}</td>))}
                        <td style={{ padding: '10px 12px', whiteSpace: 'nowrap', position: 'sticky', right: 0, background: 'var(--bg2)', zIndex: 1, borderBottom: '1px solid var(--border)' }}>
                          {(user?.role === 'admin' || user?.role === 'manager') && (
                            <>
                              {['Active', 'Pending', 'Cancelled', 'Chargeback'].map(st => (
                                s.status !== st && (<button key={st} onClick={() => changeStatus(s.id, st)} title={`Set ${st}`}
                                  style={{ padding: '3px 6px', marginRight: 2, borderRadius: 4, fontSize: 9, cursor: 'pointer', fontWeight: 600, background: (STATUS_COLORS[st] || '#888') + '22', border: `1px solid ${STATUS_COLORS[st] || '#888'}40`, color: STATUS_COLORS[st] || 'var(--muted)' }}>
                          ...
                          {user?.role === 'admin' && (<button onClick={() => deleteSale(s.id)} style={{ padding: '3px 8px', background: 'transparent', border: '1px solid rgba(248,113,113,0.3)', borderRadius: 4, color: 'var(--red)', fontSize: 10, cursor: 'pointer', marginLeft: 2 }}>Del</button>)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      {editSale && <EditModal sale={editSale} onSave={saveEdit} onCancel={() => setEditSale(null)} />}
    </div>
  );
}
